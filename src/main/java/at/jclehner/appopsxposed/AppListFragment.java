...
public class AppListFragment extends ListFragment implements LoaderManager.LoaderCallbacks<List<AppListFragment.PackageInfoData>> {
    ...
    class AppListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            PackageInfoData data = getItem(position);
            ...
            if (convertView == null) {
                ...
                holder.appLine2 = convertView.findViewById(R.id.op_name);
            ...
            if (!data.packageInfo.packageName.equals(holder.packageName)) {
                if (holder.task != null)
    ...
    static class AppListLoader extends AsyncTaskLoader<List<PackageInfoData>> {
        ...
        AppListLoader(Context context, boolean removeAppsWithUnchangedOps) {
            super(context);
        ...
        public List<PackageInfoData> loadInBackground() {
            List<PackageInfoData> data = new ArrayList<>();

            AppOpsManagerWrapper appOps = AppOpsManagerWrapper.from(getContext());
        ...
        private boolean hasUnchangedOps(PackageInfoData info) {
            AppOpsManagerWrapper appOps = AppOpsManagerWrapper.from(getContext());
            List<PackageOpsWrapper> pkgOpsList = appOps.getOpsForPackage(info.packageInfo.applicationInfo.uid,
            ...
            for (PackageOpsWrapper pkgOps : pkgOpsList) {
                for (OpEntryWrapper op : pkgOps.getOps()) {
                    if (!isUnchanged(op)) {
                        ...
                        SpannableString opSummary = new SpannableString(
        ...
        private static String[] getOpPermissions() {
            try {
                Field f = AppOpsManager.class.getDeclaredField("sOpPerms");
        ...
        private static boolean isUnchanged(OpEntryWrapper opWrapper) {
            int mode = opWrapper.getMode();
            int op = opWrapper.getOp();
            ...
            return op == AppOpsManagerWrapper.OP_WRITE_SMS && mode == AppOpsManagerWrapper.MODE_ALLOWED;
        }
    }
    ...
    @Override
    public void onLoaderReset(@NonNull Loader<List<PackageInfoData>> data) {
    ...
    private void resetChangedOps() {
        AppOpsManagerWrapper appOps = AppOpsManagerWrapper.from(getActivity());

        for (PackageInfoData pi : mAdapter.mList) {
            int uid = pi.packageInfo.applicationInfo.uid;
            String packageName = pi.packageInfo.packageName;
