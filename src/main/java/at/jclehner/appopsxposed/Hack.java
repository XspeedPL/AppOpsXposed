...
public abstract class Hack implements IXposedHookLoadPackage, IXposedHookZygoteInit {
    ...
    private static Hack[] HACKS = null;

    public static void initHacks(SharedPreferences prefs) {
        HACKS = new Hack[]{
                new BootCompletedHack(prefs),
                new FixWakeLock(prefs),
                new FixOpsPruneHack(prefs),
                //new GmsLocationHack(prefs),
                BuildConfig.DEBUG ? new DontGroupOpsHack(prefs) : null,
                new MiuiHacks(prefs),
                new GeneralHacks(prefs)
        };
    }
    ...
    public static List<Hack> getAllEnabled(boolean quiet) {
        ...
        for (Hack hack : HACKS) {
            ...
            if (hack.mPrefs.getBoolean(hack.getKey(), hack.isEnabledByDefault())) {
                hacks.add(hack);
                if (!quiet)
    ...
    public Hack(SharedPreferences prefs) {
        mPrefs = prefs;
    }
    ...
    protected final String getPreferenceString(Context context, String suffix, Object... formatArgs) {
        String name = "use_hack_" + onGetKeySuffix() + "_" + suffix;
