...
public class BootCompletedHack extends Hack {
    ...
    public BootCompletedHack(SharedPreferences prefs) {
        super(prefs);
    }
    ...
    private void patchActivityManagerService(ClassLoader classLoader) throws Throwable {
        Class<?> amServiceClazz = classLoader.loadClass(
                "com.android.server.am.ActivityManagerService");

        Class<?> procRecordClazz = classLoader.loadClass(
                "com.android.server.am.ProcessRecord");

        Class<?> iIntentRecvrClazz = classLoader.loadClass(
    ...
    private void injectLabelAndSummary(LoadPackageParam lpparam) throws Throwable {
        Class<?> appOpsStateClazz = lpparam.classLoader.loadClass(
        ...
        XposedBridge.hookAllConstructors(appOpsStateClazz, new XC_MethodHook() {
            ...
            @Override
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                CharSequence summary = OpsLabelHelper.getPermissionLabel((Context) param.args[0],
    ...
    private void addBootupTemplate(LoadPackageParam lpparam) {
        try {
            ...
            Class<?> pagerAdapterClazz = lpparam.classLoader.loadClass(
                    "com.android.settings.applications.AppOpsSummary$MyPagerAdapter");

            Class<?> opsTemplateClazz = lpparam.classLoader.loadClass(
                    "com.android.settings.applications.AppOpsState$OpsTemplate");

            Class<?> appOpsCategoryClazz = lpparam.classLoader.loadClass(
                    "com.android.settings.applications.AppOpsCategory");

            Object bootupTemplate = XposedHelpers.newInstance(opsTemplateClazz,
                    new int[]{OP_BOOT_COMPLETED}, new boolean[]{true});


            Object bootupCategoryFragment = XposedHelpers.newInstance(appOpsCategoryClazz,
                    bootupTemplate);
        ...
        try {
            ...
            XposedHelpers.findAndHookConstructor(opsTemplateClazz,
                    int[].class, boolean[].class, new XC_MethodHook() {
                        ...
                        protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                            try {
                                int oldLength = Array.getLength(param.args[0]);
                                if (oldLength == 1)
                                    return;

                                int[] newOps = new int[oldLength - 1];
                                boolean wasItemRemoved = false;
                                for (int i = 0, k = 0; i != oldLength; ++i, ++k) {
                                    int op = Array.getInt(param.args[0], i);
                                    if (op == OP_BOOT_COMPLETED) {
                                        wasItemRemoved = true;
                                        --k;
                                        continue;
    ...
    private static Map<String, Object> getAppOpsManagerFunctionsToPatch() {
        ...
        for (int api = 18; api <= Build.VERSION.SDK_INT; ++api) {
            ...
            if (i < APP_OPS_MANAGER_RETURN_VALUES.length) {
                Object[][] funcs = APP_OPS_MANAGER_RETURN_VALUES[i];
                if (funcs != null) {
