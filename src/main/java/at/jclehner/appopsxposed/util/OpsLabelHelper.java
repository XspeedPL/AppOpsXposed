...
public class OpsLabelHelper {
    ...
    private static String[] getOpLabelsOrSummaries(Context context, boolean getLabels) {
        ...
        boolean hasFakeBootCompleted = Util.isBootCompletedHackWorking() &&
        ...
        for (Field field : AppOpsManagerWrapper.class.getDeclaredFields()) {
            ...
            int op = AppOpsManagerWrapper.opFromName(opName);
            if (op == -1)
                continue;
            else if (op > maxOp)
                maxOp = op;
            ...
            String str = getAppOpsString(context, opName, getLabels);
            if (str != null)
        ...
        String[] ret = new String[AppOpsManagerWrapper._NUM_OP];
        ...
        if (maxOp + 1 != AppOpsManagerWrapper._NUM_OP) {
            for (int op = maxOp + 1; op != AppOpsManagerWrapper._NUM_OP; ++op) {
                String opName = AppOpsManagerWrapper.opToName(op);
                if (opName != null)
    ...
    private static String getAppOpsString(Context context, String opName, boolean getLabel) {
        String str = getAppOpsString(context, opName, getLabel, true);
    ...
    private static String getAppOpsString(Context context, String opName, boolean getLabel, boolean tryOther) {
        ...
        String id = "app_ops_" + (getLabel ? "labels" : "summaries") + "_" +
        ...
        try {
            if (Constants.MODULE_PACKAGE.equals(context.getPackageName()))
                ...
                res = context.getPackageManager().getResourcesForApplication("at.jclehner.appopsxposed.re");
        } catch (NameNotFoundException e) {
            Log.w("AOX", e);
            return opName;
        }

        int resId = res.getIdentifier(Constants.MODULE_PACKAGE + ":string/" + id, null, null);
        if (resId != 0) {
            try {
    ...
    private static String getFallbackString(Context context, String opName, boolean getLabel, boolean tryOther) {
        if (tryOther) {
            String other = getAppOpsString(context, opName, !getLabel, false);
            if (other != null)
                return other;
        }

        String fromPerm = opLabelFromPermission(context, opName);
        return fromPerm != null ? fromPerm : opName;
    }

    public static int getOpValue(String name) {
        try {
            Field f = AppOpsManager.class.getField(name);
    ...
    private static String opLabelFromPermission(Context context, String opName) {
        ...
        String perm = AppOpsManagerWrapper.opToPermission(op);
