...
public class AOSP extends ApkVariant {
    public AOSP(SharedPreferences prefs) {
        super(prefs);
    }
    ...
    private void addAppOpsHeader(LoadPackageParam lpparam) {
        int settingsHeadersId = Res.getSettingsIdentifier("xml/settings_headers");
    ...
    private void addAppOpsDashboardTile(LoadPackageParam lpparam) {
        final Class<?> tileClazz;
        try {
            tileClazz = XposedHelpers.findClass("com.android.settings.dashboard.DashboardTile", lpparam.classLoader);
        } catch (Throwable ignored) {
            debug("Dashboard tile class not found");
            return;
        }
