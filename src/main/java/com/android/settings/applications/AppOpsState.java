...
public class AppOpsState {
    ...
    private AppEntry getAppEntry(Context context, HashMap<String, AppEntry> appEntries,
                                 String packageName, ApplicationInfo appInfo) {
    ...
    public List<AppOpEntry> buildStateWithChangedOpsOnly() {
        List<PackageOpsWrapper> pkgs = mAppOps.getPackagesForOps(null);
        ...
        for (PackageOpsWrapper pkg : pkgs) {
            for (OpEntryWrapper op : pkg.getOps()) {
                if (op.getMode() != AppOpsManagerWrapper.MODE_ALLOWED) {
                    ...
                    AppEntry app = getAppEntry(mContext, appEntries, appInfo.packageName, appInfo);
                    if (app == null) {
                        continue;
                    }
    ...
    public List<AppOpEntry> buildState(OpsTemplate tpl, int uid, String packageName) {
        ...
        int[] opToOrder = new int[AppOpsManagerWrapper._NUM_OP];
